        ...
        [ testTask "config-path -- global tool" {
              ...
              let expectedPath =
                  if Environment.isWindows then
                      $"""{env "USERPROFILE"}\.dotnet\tools\.store\atech.ring.dotnet.cli\{pkgVer}\atech.ring.dotnet.cli\{pkgVer}\tools\net10.0\any\app.windows.toml"""
                  else if Environment.isMacOS then
                      $"""{env "HOME"}/.dotnet/tools/.store/atech.ring.dotnet.cli/{pkgVer}/atech.ring.dotnet.cli/{pkgVer}/tools/net10.0/any/app.osx.toml"""
                  else
                      $"""{env "HOME"}/.dotnet/tools/.store/atech.ring.dotnet.cli/{pkgVer}/atech.ring.dotnet.cli/{pkgVer}/tools/net10.0/any/app.linux.toml"""
          ...
          testTask "config-path -- local tool" {
              ...
              let expectedPath =
                  if Environment.isWindows then
                      $"""{env "USERPROFILE"}\.nuget\packages\atech.ring.dotnet.cli\{pkgVer}\tools\net10.0\any\app.windows.toml"""
                  else if Environment.isMacOS then
                      $"""{env "HOME"}/.nuget/packages/atech.ring.dotnet.cli/{pkgVer}/tools/net10.0/any/app.osx.toml"""
                  else
                      $"""{env "HOME"}/.nuget/packages/atech.ring.dotnet.cli/{pkgVer}/tools/net10.0/any/app.linux.toml"""

              "Config path should (default) be correct"
              |> Expect.equal actualPath expectedPath
              ...
              let expectedPath =
                  if Environment.isWindows then
                      $"""{dir.WorkPath}\.ring\settings.toml"""
                  else if Environment.isMacOS then
                      $"""/private{dir.WorkPath}/.ring/settings.toml"""
                  else
                      $"""{dir.WorkPath}/.ring/settings.toml"""
          ...
          testTask "run basic workspace in headless mode" {
              ...
              "Unexpected events sequence"
              |> Expect.sequenceEqual events [ M.RUNNABLE_STOPPED; M.RUNNABLE_DESTROYED ]
          }
